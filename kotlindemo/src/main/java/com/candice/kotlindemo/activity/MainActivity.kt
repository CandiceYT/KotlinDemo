...
import com.candice.kotlindemo.activity.complicatedwidget.StorageActivity
...
//            p0.tvItem.textSize = 15f.dip2px(DemoApplication.getInstance().applicationContext).toFloat()
            ...
            p0.tvItem.setOnClickListener {
                when (p1) {
                    //登录页面
                    0 -> LoginActivity.launch(context)
                    //视图列表
                    1 -> ViewListActivity.launch(context)
                    2 -> MaterialDesignActivity.launch(context)
                    3 -> StorageActivity.launch(context)
                    else -> Toast.makeText(context, "不做跳转", Toast.LENGTH_LONG).show()
